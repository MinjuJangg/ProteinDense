...
- Otherwise identical tokenization, padding, truncation, fuand dropout
"""
...
class Prot2TextLightCollater:
    ...
    def __call__(self, batch: List[Dict[str, str]]) -> Dict[str, torch.Tensor]:
        # ---- strict key access ----
        accessions   = [self._safe_get(item, "accession") for item in batch]
        ...
        descriptions = [self._safe_get(item, "function_text") for item in batch]
        domains_list = [item.get("domains", []) for item in batch]  # domain captions

        # ---- dropout ----
        fullnames = [
            fullname if isinstance(fullname, str) and random.random() > self.name_dropout else "unknown"
            for fullname in fullnames
        ]
        taxons = [
            taxon if isinstance(taxon, str) and random.random() > self.taxonomy_dropout else "unknown"
            for taxon in taxons
        ]
        ...
        # ---- build user messages (with domains) ----
        user_messages = []
        for fullname, taxon, seq_len, domains in zip(fullnames, taxons, sequence_lens, domains_list):
            if self.include_text_fields:
                base = f"Protein name: {fullname}; Taxon: {taxon}; "
            else:
                base = ""

            seq_placeholder = "Sequence embeddings: " + self.placeholder_token * seq_len

            if isinstance(domains, list) and len(domains) > 0:
                dense_caption = "\n".join(
                    [f"{i+1}) {d.get('caption','')}" for i, d in enumerate(domains)]
                )
                msg = f"{base}{seq_placeholder}\nDomain info:\n{dense_caption}\nFunction description:"
            else:
                msg = f"{base}{seq_placeholder}\nDomain info: None\nFunction description:"
